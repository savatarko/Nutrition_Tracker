    ...
    public List<SavedMeal> getFiltered(MealFilter mealFilter) {
        ...
        for (SavedMeal meal : list) {
            if (mealFilter.isFromHome() == true) {
                if (meal.category.equalsIgnoreCase(mealFilter.getCategory()) && meal.name.toLowerCase().contains(mealFilter.getMealName().toLowerCase()) || meal.ingredients.contains(mealFilter.getIngredient()) && (mealFilter.getMincal() ==-1 || mealFilter.getMincal() < meal.calories) && (mealFilter.getMaxcal() == -1 || mealFilter.getMaxcal() > meal.calories)) {//TODO: ovo trenutno radi samo za jedan sastojak+sastojci jos nisu ni dodati zbog jsona, ovo u filture razdvojiti zarezom
                    meals.add(meal);
                }
            } else {
                if (meal.name.toLowerCase().contains(mealFilter.getMealName().toLowerCase()) && (mealFilter.getCategory().equals("") || meal.category.equalsIgnoreCase(mealFilter.getCategory())) &&
                        (mealFilter.getArea().equals("") || meal.area.equalsIgnoreCase(mealFilter.getArea())) && (mealFilter.getIngredient().equals("") || meal.ingredients.contains(mealFilter.getIngredient())) && (mealFilter.getTag().equals("") || meal.tags.contains(mealFilter.getTag())) && (mealFilter.getMincal() ==-1 || mealFilter.getMincal() < meal.calories) && (mealFilter.getMaxcal() == -1 || mealFilter.getMaxcal() > meal.calories)) {
                    meals.add(meal);
                }
            }
        }
